// ==UserScript==
// @name Script AutoFarming Oficial
// @namespace http://brtwscripts.com
// @version 0.1
// @description Soh pros folgados kkk
// @author Gil Penner
// @include **am_farm*
// @include https://br119.tribalwars.com.br/game.php?screen=am_farm&village=9264
// @grant none
// ==/UserScript==
...
function loadSettings() {
	...
	if (farmSettings == null) {
		console.log("Criando configuraÃ§Ãµes de farm");
		farmSettings = createFarmSettings();
	} else if (farmSettings.version != farmSettingsVersion) {
		console.log("Corrigindo configuraÃ§Ãµes para a nova versÃ£o!");
...
function updateFarmSettings(farmSettings) {
	...
	for (i = 0; i < 3; i++) {
		...
		if (wall == NaN || wall == null) {
			alert("NÃ­vel mÃ¡ximo da parede para o modelo " + model.toUpperCase() + " must be set!");
			return;
		}
		...
		if (distance == NaN || distance == null) {
			alert("DistÃ¢ncia mÃ¡xima para o modelo " + model.toUpperCase() + " must be set!");
			return;
		}
	...
	saveFarmSettings(farmSettings);
	alert("ConfiguraÃ§Ãµes do assistente de fazenda salvas com sucesso!");
}

function resetSettings(farmSettings) {
	farmSettings = createFarmSettings();
	saveFarmSettings(farmSettings);
	alert("As configuraÃ§Ãµes do assistente de saque foram redefinidas para os valores padrÃ£o! A pÃ¡gina serÃ¡ atualizada agora!");
...
function createSettingsForm(farmSettings) {
	...
	fTitle.textContent = "Auto Farm ConfiguraÃ§Ãµes";
	...
	refreshTimeInputLabel.innerHTML = "Tempo de atualizaÃ§Ã£o em ms:";
	...
	timeBetweenAttacksLabel.innerHTML = "Tempo entre ataques em ms:";
	...
	switchVillagesLabel.innerHTML = "AvanÃ§ar Aldeia";
	...
	saveIcon.setAttribute('value', "Salvar ConfiguraÃ§Ãµes");
	...
	resetIcon.setAttribute('value', "Reset ConfiguraÃ§Ãµes");
